...
export class syncHelpers {
  ...
  async getNewSalesRepsData(marketersData) {
    ...
    if (unMatchedIds.length > 0) {
      const managerIds = marketersData
  ...
  async getFacilitiesDataFromSalesReps(SalesRepsData) {
      ...
      const salesRepsData = await db.execute(sql`SELECT id, ref_id FROM sales_reps WHERE ref_id IN ${salesRepsIds}`); // fetching sales reps id and ref_id from analytics db
  ...
  async getFacilitiesIds(data, salesRepsAndFacilityData){
    ...
    const facilitiesIds = data.map(item => item._id.toString()) // fetching facilitiec id 
    
    salesRepsAndFacilityData.forEach(item => {
      if (facilitiesIds.includes(item.hospital)) {
          // Find the corresponding object in data
            const matchingObject = data.find(obj => obj._id.toString() === item.hospital);
            if (matchingObject) {
                finalObj.push({
